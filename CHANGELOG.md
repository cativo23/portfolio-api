...
### Removed
- **Public `GET /profile` endpoint** — removed the controller, its response DTO, and the now-unused `ProfileService.getProfile()` data. The endpoint had no consumers: the frontend renders the CV from its own hardcoded data, and nothing ever called the BFF proxy. The chatbot grounding (`ProfileService.getGroundingProfile()`) is unaffected. (#137)
...
### Fixed
- Network `space-server_web` declared as external in production Docker Compose
