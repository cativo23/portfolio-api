import { Controller, Get, Post, Body, Patch, Param, Delete, UsePipes, ValidationPipe, Query, InternalServerErrorException, UseGuards } from '@nestjs/common';
...
export class ProjectsController {
  ...
  @ApiQuery({ name: 'is_featured', required: false, description: 'Filter by featured projects' })
  @ApiResponse({ status: 200, description: 'List of projects' })
  findAll(@Query('page') page?: string, @Query('per_page') per_page?: string, @Query('search') search?: string, @Query('is_featured') is_featured?: string) {
